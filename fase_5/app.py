...
Fluxo:
    upload -> stride_runner (mock) -> stride_parser -> friendly_agent -> render
"""
...
from core.friendly_agent import humanizar
...
from core.stride_parser import parse
from core.stride_runner import analisar_diagrama
...
def _analisar_cached(image_bytes: bytes, _content_hash: str) -> str:
    """Roda o modelo STRIDE e cacheia por hash do conteudo da imagem."""
    with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp:
        tmp.write(image_bytes)
        tmp_path = Path(tmp.name)
    try:
        return analisar_diagrama(tmp_path)
    finally:
...
def _sidebar() -> None:
    with st.sidebar:
        ...
        st.divider()
        st.caption(
            ":warning: Modo MOCK ativo: qualquer imagem retorna o relatorio "
            "de exemplo em `samples/relatorio_exemplo.txt`."
        )
    ...
    with col_relatorio:
        try:
            with st.spinner("Analisando arquitetura com o modelo STRIDE..."):
                raw = _analisar_cached(image_bytes, content_hash)
