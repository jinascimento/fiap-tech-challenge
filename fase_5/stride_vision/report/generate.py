...
def build_markdown_report(image_name: str, graph: nx.DiGraph, threats: list[Threat]) -> str:
    ...
    if skipped:
        lines.append(f"({skipped} seta(s) detectada(s) sem keypoints de direcao suficientes - ignoradas na reconstrucao do grafo)")
    ...
    for category in sorted(by_category):
        ...
        for t in items:
            lines.append(f"{t.target_label} (component#{t.target_id}): {t.description}")
