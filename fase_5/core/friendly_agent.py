...
Usa por padrao o Gemini 2.5 Flash via Google AI Studio (provider ``google``),
mas pode ser trocado temporariamente para Claude (provider ``anthropic``) via
a variavel de ambiente ``LLM_PROVIDER`` -- util para testar sem esbarrar em
rate limit do Gemini. Requer, conforme o provider escolhido:

- ``google``: ``GEMINI_API_KEY`` (ou ``GOOGLE_API_KEY``)
- ``anthropic``: ``ANTHROPIC_API_KEY``

Carrega o ``.env`` neste modulo (nao depende da ordem de import em :mod:`app`,
que so chama ``load_dotenv()`` depois de importar este arquivo).
...
def _model_id() -> str:
    ...
    return f"google:{DEFAULT_GEMINI_MODEL}"
...
def humanizar(rel: RelatorioTecnico) -> RelatorioAmigavel:
    ...
    Raises:
        RuntimeError: Se a chave de API do provider selecionado nao estiver definida.
    """
    if LLM_PROVIDER == "anthropic":
        if not os.getenv("ANTHROPIC_API_KEY"):
            raise RuntimeError(
                "ANTHROPIC_API_KEY nao esta definida. Copie .env.example para "
                ".env e cole sua chave da Anthropic (https://console.anthropic.com/settings/keys)."
            )
