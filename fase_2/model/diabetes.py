...
def prepare_dataset(df):
    ...
    df_processed = pd.get_dummies(
        df_processed, columns=["gender", "smoking_history", "age_bins"], drop_first=True
    )
    return df_processed
...
def load_dataset():
    ...
    dataset_path = os.path.join(
        base_path, "..", "data", "diabetes_prediction_dataset.csv"
    )
...
def train_mlp(df_processed, mlp_params, test_size=0.2, random_state=42):
    ...
    return mlp_model, scaler, X_test, y_test
